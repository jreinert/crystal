...
describe ByteFormat do
  describe "little endian" do
    describe "encode" do
      it "writes int8" do
        io = StringIO.new
        io.write_object 0x12_i8, ByteFormat::LittleEndian
        assert_bytes io, 0x12
      end

      it "writes int16" do
        io = StringIO.new
        io.write_object 0x1234_i16, ByteFormat::LittleEndian
        assert_bytes io, 0x34, 0x12
      end

      it "writes uint16" do
        io = StringIO.new
        io.write_object 0x1234_u16, ByteFormat::LittleEndian
        assert_bytes io, 0x34, 0x12
      end

      it "writes int32" do
        io = StringIO.new
        io.write_object 0x12345678_i32, ByteFormat::LittleEndian
        assert_bytes io, 0x78, 0x56, 0x34, 0x12
      end

      it "writes int64" do
        io = StringIO.new
        io.write_object 0x123456789ABCDEF0_i64, ByteFormat::LittleEndian
        assert_bytes io, 0xF0, 0xDE, 0xBC, 0x9A, 0x78, 0x56, 0x34, 0x12
      end

      it "writes float32" do
        io = StringIO.new
        io.write_object 1.234_f32, ByteFormat::LittleEndian
        assert_bytes io, 0xB6, 0xF3, 0x9D, 0x3F
      end

      it "writes float64" do
        io = StringIO.new
        io.write_object 1.234, ByteFormat::LittleEndian
  ...
  describe "big endian" do
    describe "encode" do
      it "writes int8" do
        io = StringIO.new
        io.write_object 0x12_i8, ByteFormat::BigEndian
        assert_bytes io, 0x12
      end

      it "writes int16" do
        io = StringIO.new
        io.write_object 0x1234_i16, ByteFormat::BigEndian
        assert_bytes_reversed io, 0x34, 0x12
      end

      it "writes int32" do
        io = StringIO.new
        io.write_object 0x12345678_i32, ByteFormat::BigEndian
        assert_bytes_reversed io, 0x78, 0x56, 0x34, 0x12
      end

      it "writes int64" do
        io = StringIO.new
        io.write_object 0x123456789ABCDEF0_i64, ByteFormat::BigEndian
        assert_bytes_reversed io, 0xF0, 0xDE, 0xBC, 0x9A, 0x78, 0x56, 0x34, 0x12
      end

      it "writes float32" do
        io = StringIO.new
        io.write_object 1.234_f32, ByteFormat::BigEndian
        assert_bytes_reversed io, 0xB6, 0xF3, 0x9D, 0x3F
      end

      it "writes float64" do
        io = StringIO.new
        io.write_object 1.234, ByteFormat::BigEndian
    ...
    describe "decode" do
      it "reads int8" do
        io = SliceIO.new(0x12)
        int = io.read_object Int8, ByteFormat::BigEndian
        int.should eq(0x12_i8)
      end

      it "reads int16" do
        io = SliceIO.new(0x12, 0x34)
        int = io.read_object Int16, ByteFormat::BigEndian
        int.should eq(0x1234_i16)
      end

      it "reads unt16" do
        io = SliceIO.new(0x12, 0x34)
        int = io.read_object UInt16, ByteFormat::BigEndian
        int.should eq(0x1234_u16)
      end

      it "reads int32" do
        io = SliceIO.new(0x12, 0x34, 0x56, 0x78)
        int = io.read_object Int32, ByteFormat::BigEndian
        int.should eq(0x12345678_i32)
      end

      it "reads int64" do
        io = SliceIO.new(0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0)
        int = io.read_object Int64, ByteFormat::BigEndian
        int.should eq(0x123456789ABCDEF0_i64)
      end

      it "reads float32" do
        io = SliceIO.new(0x3F, 0x9D, 0xF3, 0xB6)
        float = io.read_object Float32, ByteFormat::BigEndian
        float.should be_close(1.234, 0.0001)
      end

      it "reads float64" do
        io = SliceIO.new(0x3F, 0xF3, 0xBE, 0x76, 0xC8, 0xB4, 0x39, 0x58)
        float = io.read_object Float64, ByteFormat::BigEndian
        float.should be_close(1.234, 0.0001)
      end
    end
  end
